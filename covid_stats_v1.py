...
#                            verbose=0)
    ...
    res_seir = optimize.least_squares(fobj_seir,np.log10(np.array([1E-1,1E-2,1E0,I0_fit])), 
                                      jac='3-point',
#                                      bounds=([-50,-50,-15,0],[50,50,50,50]),
...
#    popt = res.x
    popt_log = res_seir.x
    popt_seir = np.power(10, popt_log)
...
#                           res, mode='observation')
    ...
    ysize = len(res_seir.fun)
    cost = 2 * res_seir.cost  # res.cost is half sum of squares!
    s_sq = cost / (ysize - popt_seir.size)
    
    # Do Moore-Penrose inverse discarding zero singular values.
    _, s, VT = svd(res_seir.jac, full_matrices=False)
    threshold = np.finfo(float).eps * max(res_seir.jac.shape) * s[0]
    s = s[s > threshold]
    VT = VT[:s.size]
...
#    print(pcov_log)
    ...
    if pcov_log is None:
        # indeterminate covariance
        print('Failed estimating pcov_seir')
        pcov_log = np.zeros((len(pcov_log), len(pcov_log)), dtype=float)
        pcov_log.fill(np.inf)
        
#    pcov_seir = np.power(10, pcov_log)
    
    
    ci_seir = np.power(10,cf.confint(ysize,popt_log,pcov_log))
    def smodel(x,p): return seir(x,p,population,I0_fit,R0_fit)
    ypred_seir,lpb_seir,upb_seir = cf.predint_multi(xpred, time_days_fit, 
                                           infected_fit, smodel,
                                           res_seir, mode='observation')
    
    
    
    
    R0_cal = popt_seir[0]/popt_seir[1]
...
#             zorder=5)
    ...
    res_str = '$\\beta$ = {0} (days)$^{{-1}}$\n95% CI: [{1},{2}]\n'.format(latex_format(popt_seir[0],3),
                  latex_format(ci_seir[0][0],3), latex_format(ci_seir[0][1],3))
    res_str += '$\\gamma$ = {0} (days)$^{{-1}}$\n95% CI: [{1},{2}]\n'.format(latex_format(popt_seir[1],3),
                    latex_format(ci_seir[1][0],3), latex_format(ci_seir[1][1],3))
    res_str += '$\\sigma$ = {0} (days)$^{{-1}}$\n95% CI: [{1},{2}]\n'.format(latex_format(popt_seir[2],3),
                    latex_format(ci_seir[2][0],3), latex_format(ci_seir[2][1],3))
#    res_str += '$I_0 =$  {0:.3g}\n95% CI: [{1:.3g},{2:.3g}]'.format(popt_seir[2],
#                    ci_seir[2][0], ci_seir[2][1])
    res_str += '$I_0 =$  {0:.3g}, $R_0$ = {1:.0f}, $E_0 =$ {2:.2g}, $t_0 =$ {3:.0f}'.format(I0_fit, 
                         R0_fit, popt_seir[3], time_days_fit[0])
