...
results_folder = './results_infected'
...
def fobj(p: np.ndarray, time: np.ndarray, cases: np.ndarray):
    return p[0]*np.exp(p[1]*time) + p[2] - cases

def jac(p: np.ndarray, time: np.ndarray, cases: np.ndarray):
    jm = np.empty((len(time),3))
    for i, t in enumerate(time):
        jm[i] = (np.exp(p[1]*t), p[0]*t*np.exp(p[1]*t), 1.0)
    return jm

def model(time: np.ndarray, p: np.ndarray):
    return p[0]*np.exp(p[1]*time) + p[2]
...
def fobj_sir(p: np.ndarray, time: np.ndarray, cases: np.ndarray, population: float, 
    ...
    y = sol.sol(time)
    S, I, R = y
#    return np.log10(I) - np.log10(cases)
    return I - cases
...
def sir_IR(time: np.ndarray, p: np.ndarray, population: float, I0_: int, R0_: int = 0):
    sol = sir_model.sir_model(time, N=population, beta=p[0], gamma=p[1],
                              I0=I0_, R0=R0_)
    y = sol.sol(time)
    S, I, R = y
...
if __name__ == '__main__':
    ...
    df_confd_country = df_confd_country[df_confd_country.columns[2:-1]].T.reset_index()
    ...
    df_death_country = df_death_country[df_death_country.columns[2:-1]].T.reset_index()
    ...
    df_recvd_country = df_recvd_country[df_recvd_country.columns[2:-1]].T.reset_index()
...
#    new_cases = covid_by_country['new_cases'].to_numpy()
    ...
    I0 = infected[0]
    id_start = np.argmin(infected<=I0) - 1
    range_msk = np.zeros_like(infected, dtype=bool)
...
#                            verbose=0)
    ...
    res_sir = optimize.least_squares(fobj_sir, np.array([1.0,0.01]), jac='3-point',
                                     bounds=([1E-50,1E-50], [1E10,1E10]),
                                     args=(time_days_fit,infected_fit,population,
                                           I0_fit, R0_fit),
                                     xtol=all_tol*1,
                                     ftol=all_tol*1,
                                     gtol=all_tol*1,
...
#        pcov.fill(np.inf)
    ...
    xpred = np.linspace(np.amin(time_days_fit), np.amax(time_days_fit), 200)
    xpred_2 = np.linspace(0, np.amax(time_days)+add_days, len(time_days)+add_days, 200)
    ypred_2 = sir(xpred_2, popt_sir, population,I0_fit)
    ipred,rpred = sir_IR(xpred, popt_sir, population,I0_fit,R0_fit)
...
#                           res, mode='observation')
    ...
    ypred_sir,lpb_sir,upb_sir = cf.predint(xpred, time_days_fit, 
                                           infected_fit, smodel,
...
#    ax1.plot(xpred, ypred, color=data_color)
    ...
    if plot_pbands:
        ax2.fill_between(xpred,lpb_sir,upb_sir, color=pband_color)
    ax2.plot(time_days_fit, infected_fit,'o', color=data_color, fillstyle='none', label='infected')
    ax2.plot(xpred, ypred_sir, color=model2_color)
    
    
    
    ax2b = ax2.twinx()
    ax2b.tick_params(axis='y', labelcolor='C4')
    ax2b.plot(xpred,rpred,ls='--',color='C4')
    
    
    ax3.plot(xpred_2, ypred_2, color='C0')
    imax_sir = np.amax(ypred_2)
    tmax_sir = int(xpred_2[ypred_2 == imax_sir])
    ...
    ax3.annotate(peak_str,
            ...
            xytext=(xpos, -50), textcoords='offset points',
            color='tab:red',
            fontsize=12,
    ...
    ax2b.set_ylabel('Removed (#)', color='C4')
...
#                    ci_sir[2][0], ci_sir[2][1])
    res_str += '$I_0 =$  {0:.3g}, $R_0$ = {1:d}'.format(I0_fit, R0_fit)
    
    ax2.text(0.05, 0.95, res_str, 
             horizontalalignment='left',
             verticalalignment='top', 
...
#    ax2.yaxis.set_ticks_position('both')
    ...
    ax2b.yaxis.set_major_formatter(engfmt)
...
#    plt.tight_layout(pad=0.0, w_pad=0.0, h_pad=0.0)
    ...
    filetag = 'covid19_model_{0}'.format(location)
